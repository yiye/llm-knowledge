...
- **注意**: 本分析基于 2026 年 3 月的代码，可能随版本更新而变化
...
### 1.8 扩展 API：requestHeartbeatNow、onAgentEvent、onSessionTranscriptUpdate

**核心文件**：`src/plugins/runtime/runtime-system.ts`、`src/plugins/runtime/runtime-events.ts`、`src/plugins/runtime/types-core.ts`

**requestHeartbeatNow**（`runtime.system.requestHeartbeatNow`）：
- 扩展在入队系统事件后调用，可立即请求 Heartbeat 唤醒目标 Session
- 签名：`(opts?: { reason?: string; coalesceMs?: number; agentId?: string; sessionKey?: string }) => void`
- 典型用法：`enqueueSystemEvent` 后调用 `requestHeartbeatNow({ reason: "exec-event", sessionKey })`，使 exec 完成等事件尽快被 Agent 处理

**onAgentEvent**（`runtime.events.onAgentEvent`）：
- 订阅 Agent 生命周期/流式事件（lifecycle、tool、assistant、compaction 等）
- 回调参数：`AgentEventPayload`（runId、seq、stream、ts、data、sessionKey）
- 来源：`src/infra/agent-events.js`

**onSessionTranscriptUpdate**（`runtime.events.onSessionTranscriptUpdate`）：
- 订阅 Session Transcript 更新（新消息写入 JSONL 时触发）
- 回调参数：`{ sessionFile: string }`
- 来源：`src/sessions/transcript-events.js`
- 典型用途：Memory 索引监听 transcript 变更以触发增量索引

**CHANGELOG 引用**：Plugin runtime/events: expose `runtime.events.onAgentEvent` and `runtime.events.onSessionTranscriptUpdate` for extension-side subscriptions (#16044)
...
### 3.7 ClawHub 插件市场（⭐ 2026.3.22 正式确认）

**ClawHub 是什么？**

ClawHub 是 OpenClaw 的官方插件和 Skills 市场（类似 npm），用户可以浏览、搜索、安装、发布 Skills 和插件。这之前是推测，**2026.3.22 更新确认了其实际实现**。

**确认的功能**（基于代码 `src/gateway/server-methods/skills.ts`）：

#### 安装和更新（已确认）

- **CLI 命令**（已在 CHANGELOG 2026.3.22 确认）：
  - `openclaw skills install <name>` - 从 ClawHub 安装 Skill
  - `openclaw plugins install <name>` - 从 ClawHub 安装插件
  - `openclaw skills update` - 更新所有已安装的 Skills
  - `openclaw plugins update` - 更新所有已安装插件

#### 底层机制（已确认）

安装流程：CLI → Gateway RPC (`skills.install`) → 从 ClawHub 拉取 → 写入本地目录 → `bumpSkillsSnapshotVersion` 热刷新，全程无需重启 Gateway。
...
4. **Session 启动日期锚定**：在 post-compaction、memory-flush 等上下文中将 `YYYY-MM-DD` 占位符替换为运行时日期（`src/auto-reply/reply/post-compaction-context.ts`、`memory-flush.ts`），确保 AI 能正确引用 `memory/YYYY-MM-DD.md`，提升主动记忆召回的准确性。
...
- `src/memory/hybrid.ts` - Hybrid Search 合并算法（含 MMR + Temporal Decay）
- `src/memory/manager-search.ts` - Vector 和 Keyword 搜索实现
- `src/memory/qmd-manager.ts` - QMD 外部后端
- `src/memory/search-manager.ts` - 后端切换入口（builtin vs QMD）
...
除了 Heartbeat（自动定时）和用户消息（被动响应），Cron 任务是第三种 AI 主动性触发源：

- 通过 `cron.add` RPC 配置（cron expression）
- 支持 `lightContext: true` 轻量引导上下文（节省 token）
- 支持每个 Job 独立 `model` / `fallbacks` / `timeoutSeconds`
- 支持 `deliver: true` 将结果投递到指定 `channel` / `to`
- 通过 `externalContentSource` 追溯 HTTP webhook 触发来源

```
触发源①  用户消息     → Chat Apps（WhatsApp / Telegram / Slack...）
触发源②  Heartbeat   → 定时 30m 主动唤醒（Active Hours 感知）
触发源③  Cron Tasks  → 用户配置 cron expression，指定 Agent + 任务
```

**Memory 搜索增强**：

- **MMR（Maximum Marginal Relevance）**：避免搜索结果冗余，在相关性和多样性间取平衡
- **Temporal Decay（时间衰减）**：近期记录权重更高，旧记录权重递减
- **QMD 外部后端**：支持外部 CLI 作为 Memory 存储，内置 SQLite 后端作为自动降级

**Embedding 提供商扩展**：原来仅支持 OpenAI/Gemini/Local，现在新增 **Voyage**、**Mistral**、**Ollama**（共 6 种）。
